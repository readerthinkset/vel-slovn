"""
YouTube Upload Script - VELOCITY SLOVENIAN
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Slovenian Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Slovenian with VELOCITY SLOVENIAN!", "", f"Category: {category}", "", f"Master Slovenian one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('slovenian', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily slovenian lessons!", "", f"#LearnSlovenian #SlovenianLessons #SlovenianForBeginners #LanguageLearning", f"#Slovenian #Education #Tutorial #DailySlovenian #{category.replace(' ', '')}", f"#VELOCITYSLOVENIAN #SlovenianPhrases #SpeakSlovenian"])
    tags = [f"learn slovenian", f"slovenian lessons", f"slovenian for beginners", f"slovenian phrases", "language learning", f"speak slovenian", category.lower(), "education", f"daily slovenian", "velocity slovenian", f"slovenian learning"]
    return title, "\n".join(lines), tags
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Slovenian Daily", description=f"#shorts #learnslovenian #slovenian #language #education", tags=[hashtag, 'education', 'language learning'], category_id='22')
